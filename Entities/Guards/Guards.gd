...
func Damage(power:float) -> void:
	...
	if health	<= 0:
		var corpse = load("res://Entities/Guards/Guard-Corpse/Guard-Corpse.tscn").instance()
...
func CleanTriggerArea() -> void:
	if get_node_or_null(custom_trigger) == null:
		$TriggerArea.queue_free()
	else:
		get_node(custom_trigger).disconnect("body_entered",self,"_on_TriggerArea_body_entered")
