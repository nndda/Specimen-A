...
func InitVisualLoading() -> void:
	...
	current_objects_list.sort_custom(
		func( a, b ): return (
			a.distance_to( glbl.head_pos ) >
			b.distance_to( glbl.head_pos ) )
		)
	...
	tween.tween_property(
		...
		1.0, glbl.current_objects.size() * 0.6 )
	tween.connect(
		"finished",
		Callable( self, "FinishedVisualLoading" ) )
	tween.play()

func FinishedVisualLoading() -> void:

	print( "glbl.current_objects" )
	print( glbl.current_objects )
